...
![@JSSE 的参考手册](android网络请求-证书认证/1523800685580.png)
...
-   服务器端用server.csr文件向CA申请证书，签名过程需要CA的公钥证书和私钥参与，最终颁发一个带有CA签名的服务器端证书server.crt：
    openssl x509 -req -CA ca\_public.crt -CAkey ca\_private.key -CAcreateserial -in server.csr -out server.crt
...
建议开发或对https方面不了解的安全测试同学查看android开发指南之[通过HTTPS 和 SSL确保安全](https://developer.android.com/training/articles/security-ssl.html)。
